...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="update_purchaseorder_list_table">
	...
	<tr class="classnot">
        ...
        <td >
            <?php 
            if($info_arr['individualArt']){
                $classinvoice = 'save_invoice_article';
                $classreceipt = 'save_receipt_article';
            }else{
                $classinvoice = 'save_invoice';
                $classreceipt = 'save_receipt';
            }
            if($trans->checkout_status == 0): ?>
				<a id="<?php echo $classinvoice;?>" class="save_mail_bill" name="<?php echo $trans->id ?>"><?php echo __('faktura')?></a>
			<?php endif;?>
			<?php if($trans->checkout_status == 1): ?>
				<a id="<?php echo $classreceipt;?>" class="save_mail_bill" name="<?php echo $trans->id ?>"><?php echo __('kvitto')?></a>
			<?php endif;?>
        </td>
        <td>
