<form name="update_sbt_artiklar" id="update_sbt_artiklar" method="post" action="">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" id="update_sbt_article_list_table">
    ...
    <tr id="<?php echo 'row_'.$article->id; ?>" class="classnot">
      ...
      <td class="<?php echo $class ?>"> [ <a class="<?php echo $class ?>" title="Editera denna artikel" href="<?php echo 'https://'.$host_str.'/sbt/editAnalysis/article_id/'.$article->id; ?>">E</a> | <a id="delete_sbt_article" class="<?php echo $class ?>" title="Ta bort <?php echo $article->id ?>" href="javascript:open_confirmation('Vill du verkligen ta bort artikel med rubriken <?php echo $article->analysis_title ?> article id: <?php echo $article->id; ?>','<?php echo $article->id; ?>','delete_sbt_article_confirm_box','delete_sbt_article_msg')">X</a> ] <a class="<?php echo $class ?>" title="G till denna artikel!" href="<?php echo 'https://'.$host_str.'/sbt/sbtArticleDetails/article_id/'.$article->id; ?>">&gt;&gt;</a> </td>
