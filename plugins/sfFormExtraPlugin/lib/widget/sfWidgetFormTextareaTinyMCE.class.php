...
class sfWidgetFormTextareaTinyMCE extends sfWidgetFormTextarea
{
  ...
  {
    //$id = $this->generateId($name).'_'. time();
...
EOF
    ...
    if(!sfContext::getInstance()->getRequest()->isXmlHttpRequest()){
        $js = $js. '<script type="text/javascript">renderTinyMce();</script>';
    }else{
        $js = $js. '<script type="text/javascript">setTimeout("renderTinyMce()",50);</script>';
    }
